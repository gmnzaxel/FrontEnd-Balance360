...
const useCart = ({
  ...
  const [discount, setDiscount] = useState(() => localStorage.getItem(discountKey) || '')
        ...
        const descItem =
          !item.discountValue || isNaN(dv)
            ...
            : item.discountType === '%'
              ? baseSub * (dv / 100)
              : dv
        return acc + Math.max(0, baseSub - descItem)
      }, 0),
    [cart],
  )

  const total = useMemo(() => {
    if (!discount) return Math.max(0, subtotal)
    const dv = parseFloat(discount)
    if (isNaN(dv) || dv <= 0) return Math.max(0, subtotal)
